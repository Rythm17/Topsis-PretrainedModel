...
def topsis(data, weights, impacts):
    ...
    metrics = ['Accuracy', 'Precision', 'Recall', 'F1 Score', 'BLEU Score', 'ROUGE Score', 'Perplexity']
...
def main():
    try:
        ...
        if len(weights) != 7:
            raise ValueError("Weights must be provided for 7 metrics (Accuracy, Precision, Recall, F1 Score, BLEU Score, ROUGE Score, Perplexity).")
        
        weights = [w / sum(weights) for w in weights]
        weights_dict = dict(zip(['Accuracy', 'Precision', 'Recall', 'F1 Score', 'BLEU Score', 'ROUGE Score', 'Perplexity'], weights))

        impacts = impacts_input.split(',')
        if len(impacts) != 7:
            raise ValueError("Impacts must be provided for 7 metrics (Accuracy, Precision, Recall, F1 Score, BLEU Score, ROUGE Score, Perplexity).")
        
        impacts_dict = dict(zip(['Accuracy', 'Precision', 'Recall', 'F1 Score', 'BLEU Score', 'ROUGE Score', 'Perplexity'], impacts))
